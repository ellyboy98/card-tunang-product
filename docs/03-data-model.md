...
```ts
type ScheduleItem = { time: string; label: string };          // time ≤ 20, label ≤ 120
type Contact      = { name: string; relation?: string; phone: string }; // phone: Malaysian, digits only after normalisation
```
